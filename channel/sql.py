...
def get_user_recommend(user_id):
	with connection.cursor() as cursor:
		...
		cursor.execute("SELECT DISTINCT channel_id FROM channel_tags WHERE tag_id IN(SELECT tag_id FRM user_follows_tag WHERE user_id = %s) AND channel_id NOT IN (SELECT channel_id FROM user_follows_channel WHERE user_id = %s)", [user_id, user_id])
